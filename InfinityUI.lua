...
if getgenv().success then
    ...
    local Wtvp = game:GetService("Workspace").CurrentCamera.worldToViewportPoint
    getgenv().brin = false
    
    if game:GetService("CoreGui"):FindFirstChild("TR1V5Ui") then
        game:GetService("Players").LocalPlayer:Kick("You already executed TR1V5")
    end
    
    local downedplayers = {}
    local playerlist = {}
    for i,v in pairs(game.Players:GetChildren()) do
        if v.Character ~= nil and v.Character:FindFirstChild("HumanoidRootPart") and v ~= localplayer then
            table.insert(playerlist, v.Name)
        end
    end

    --------------------------------
    local function bringit(plrname)
        if game.Players[plrname].Character ~= nil and getgenv().brin == false then
            getgenv().brin = true
            local oldcf = localplayer.Character.HumanoidRootPart.CFrame
            task.spawn(function()
            while getgenv().brin and task.wait() do
                localplayer.Character.HumanoidRootPart.CFrame = game.Players[plrname].Character.HumanoidRootPart.CFrame+Vector3.new(0,-7,0)
            end
            end)
            wait(1.5)
            game:GetService("ReplicatedStorage"):WaitForChild("Events"):WaitForChild("Character"):WaitForChild("Interact"):FireServer("Revive", plrname)
            localplayer.Character.HumanoidRootPart.CFrame = localplayer.Character.HumanoidRootPart.CFrame+Vector3.new(0,7,0)
            getgenv().brin = false
            wait(.5)
            localplayer.Character.HumanoidRootPart.CFrame = oldcf
            wait(.5)
            game:GetService("ReplicatedStorage").Events.Revive.CarryPlayer:FireServer(plrname)
        end
    end
...
if IKAI then
    ...
    -- Load UI Library
    local library = library1
    
    -- Create main window
    local window = library:Window("TR1V5 Hub", "Evade")
    ...
    -- Auto Farm Features
    AutoFarmTab:Toggle("Auto Revive", getgenv().config.AutoRevive, function(value)
        getgenv().config.AutoRevive = value
    end)

    AutoFarmTab:Toggle("Auto Respawn", getgenv().config.AutoRespawn, function(value)
        getgenv().config.AutoRespawn = value
    end)

    AutoFarmTab:Toggle("Auto Objectives", getgenv().config.AutoObjectives, function(value)
        getgenv().config.AutoObjectives = value
    end)

    AutoFarmTab:Button("Respawn", function()
        game:GetService("ReplicatedStorage").Events.Respawn:FireServer()
    end)

    -- Teleportation Features
    AutoFarmTab:Label("Downed Player Bringer")
    local bringDropdown = AutoFarmTab:Dropdown("Select Player", nil, downedplayers, 'Single',function(selected)
        if localplayer.Character and localplayer.Character:FindFirstChild("HumanoidRootPart") then
            bringit(selected)
        end
    end)

    AutoFarmTab:Label("Teleport to Player")
    local teleportDropdown = AutoFarmTab:Dropdown("Select Player", nil, playerlist, 'Single',function(selected)
    ...
    -- Visuals Features
    VisualsTab:Toggle("Monster Box ESP", getgenv().config.BoxESPMonster, function(value)
        getgenv().config.BoxESPMonster = value
    end)

    VisualsTab:Toggle("Line ESP", getgenv().config.LineESP, function(value)
        getgenv().config.LineESP = value
    end)

    VisualsTab:Toggle("Name ESP", getgenv().config.NameESP, function(value)
        getgenv().config.NameESP = value
    end)

    VisualsTab:Toggle("Box ESP", getgenv().config.BoxESP, function(value)
        getgenv().config.BoxESP = value
    end)

    -- Player Features
    MiscTab:Slider("WalkSpeed", 1450, 12000, getgenv().config.WalkSpeed, function(value)
        getgenv().config.WalkSpeed = value
    end)

    MiscTab:Slider("JumpPower", 3, 20, getgenv().config.JumpPower, function(value)
        getgenv().config.JumpPower = value
    end)

    MiscTab:Toggle("Infinite Jump", getgenv().config.InfJump, function(value)
        getgenv().config.InfJump = value
    end)

    MiscTab:Toggle("Bunny Hop", getgenv().config.BHop, function(value)
        getgenv().config.BHop = value
    end)

    MiscTab:Toggle("No Camera Shake", getgenv().config.NoCameraShake, function(value)
        getgenv().config.NoCameraShake = value
    end)

    MiscTab:Toggle("Time Notifier", getgenv().config.TimeNotifier, function(value)
        getgenv().config.TimeNotifier = value
    end)

    MiscTab:Button("First Person Camera", function()
        if localplayer.Character and localplayer.Character:FindFirstChild("WeaponController") then
            local cm = getsenv(localplayer.Character.WeaponController)
            cm.onClient("FPCamera")
        end
    end)

    MiscTab:Button("Third Person Camera", function()
        if localplayer.Character and localplayer.Character:FindFirstChild("WeaponController") then
            local cm = getsenv(localplayer.Character.WeaponController)
            cm.onClient("TPCamera", {Zoom = 15})
        end
    end)

    MiscTab:Button("Enable Chat", function()
        if localplayer.PlayerGui:FindFirstChild("Chat") then
            localplayer.PlayerGui.Chat.Frame.ChatBarParentFrame.Position = UDim2.new(0,0,1,-42)
            localplayer.PlayerGui.Chat.Frame.ChatChannelParentFrame.Position = UDim2.new(0,0,0,2)
            localplayer.PlayerGui.Chat.Frame.ChatChannelParentFrame.Visible = true
        end
    end)

    MiscTab:Button("Disable Chat", function()
        if localplayer.PlayerGui:FindFirstChild("Chat") then
            localplayer.PlayerGui.Chat.Frame.ChatBarParentFrame.Position = UDim2.new(0,4,0,4)
    ...
    task.spawn(function()
        while task.wait(1) do
            -- Update downed players dropdown
            local currentDowned = {}
            for i,v in pairs(game.Players:GetChildren()) do
                if v.Character and v.Character:FindFirstChild("HumanoidRootPart") and 
                   v.Character:GetAttribute("Downed") and not v.Character:FindFirstChild("CarriedBy") and 
                   v ~= localplayer then
                    table.insert(currentDowned, v.Name)
                end
            end
            bringDropdown:Refresh(currentDowned, true)
            ...
            teleportDropdown:Refresh(currentPlayers, true)
        end
    end)
    ...
    local function getobj()
        ...
        local objtab = {"Transportation", "Switch", "Generator"}
        if game:GetService("Workspace").Game:FindFirstChild("Map") and 
           game:GetService("Workspace").Game:FindFirstChild("Map"):FindFirstChild("Parts") and 
           game:GetService("Workspace").Game:FindFirstChild("Map"):FindFirstChild("Parts"):FindFirstChild("Objectives") then
            for i,v in pairs(game:GetService("Workspace").Game:FindFirstChild("Map"):FindFirstChild("Parts"):FindFirstChild("Objectives"):GetChildren()) do
                if v:FindFirstChild("Use") and table.find(objtab, v.Name) and 
                   localplayer.Character and localplayer.Character:FindFirstChild("HumanoidRootPart") and 
                   v:FindFirstChildOfClass("MeshPart") and v:GetAttribute("Completed") == false then
                    local mag = (v:FindFirstChildOfClass("MeshPart").Position - localplayer.Character.HumanoidRootPart.Position).magnitude
                    if mag < tardis then
                        tardis = mag
                        tar = v
                    end
    ...
    task.spawn(function()
        while task.wait() do
            if getgenv().config.AutoObjectives and localplayer.Character and 
               localplayer.Character:FindFirstChild("HumanoidRootPart") and 
               not localplayer.Character:GetAttribute("Downed") and getobj() ~= nil and 
               getobj():WaitForChild("Use") and game:GetService("Workspace").Game.Stats:GetAttribute("RoundStarted") == true and 
               localplayer.PlayerGui:FindFirstChild("HUD") and localplayer.PlayerGui:FindFirstChild("HUD"):FindFirstChild("Objectives") and 
               localplayer.PlayerGui:FindFirstChild("HUD"):FindFirstChild("Objectives"):FindFirstChild("Frame") and 
               localplayer.PlayerGui:FindFirstChild("HUD"):FindFirstChild("Objectives"):FindFirstChild("Frame"):FindFirstChild("Objective") and 
               localplayer.PlayerGui:FindFirstChild("HUD"):FindFirstChild("Objectives"):FindFirstChild("Frame"):FindFirstChild("Objective").TextColor3 ~= Color3.fromRGB(137, 255, 141) then
                pcall(function()
                    localplayer.Character.HumanoidRootPart.CFrame = getobj():FindFirstChildOfClass("MeshPart").CFrame+Vector3.new(0,-3,0)
                    wait(1)
                    getobj().Use:FireServer(false)
                    if getobj().Name == "Generator" then 
                        wait(8)
                    elseif getobj().Name == "Transportation" then
                        wait(3)
                    elseif getobj().Name == "Switch" then
                        wait(1)
                    end
                    getobj().Use:FireServer(true)
                    getobj().Use:FireServer(true)
                    getobj().Use:FireServer(true)
                    getobj().Use:FireServer(true)
                    getobj().Use:FireServer(true)
    ...
    task.spawn(function()
        while true and task.wait(1) do
            if getgenv().config.TimeNotifier and game:GetService("Workspace").Game.Stats:GetAttribute("RoundStarted") == true then
                -- Time notifier logic
            end
        end
    end)
    ...
    task.spawn(function()
        while true and task.wait() do
            if getgenv().config.NoCameraShake then
    ...
    task.spawn(function()
        while true and task.wait(1) do
            if getgenv().config.BHop then
                if localplayer.Character ~= nil and localplayer.Character:FindFirstChild("Humanoid") then
    ...
    local function Getdownplayers()
        local target = nil
        local tardis = math.huge
        for i,v in pairs(game.Players:GetChildren()) do
            if v.Character ~= nil and v.Character:FindFirstChild("HumanoidRootPart") and v.Character:GetAttribute("Downed") and not v.Character:FindFirstChild("CarriedBy") and not v:GetAttribute("Dead") then
    ...
    local function Getdownplayerss()
        local target = nil
        local tardis = 0
        for i,v in pairs(game.Players:GetChildren()) do
            if v.Character ~= nil and v.Character:FindFirstChild("HumanoidRootPart") and v.Character:GetAttribute("Downed") and not v.Character:FindFirstChild("CarriedBy") and not v:GetAttribute("Dead") and localplayer.Character ~= nil and localplayer.Character:FindFirstChild("HumanoidRootPart") then
                local mag = (v.Character.HumanoidRootPart.Position - localplayer.Character.HumanoidRootPart.Position).magnitude
                if mag > tardis then
                    tardis = mag
                    target =  v.Character
    ...
    task.spawn(function()
        while task.wait() do
            if getgenv().config.AutoRevive then
                if Getdownplayerss() ~= nil and Getdownplayerss():FindFirstChild("HumanoidRootPart") and localplayer.Character ~= nil and localplayer.Character:FindFirstChild("HumanoidRootPart") then
                    localplayer.Character.HumanoidRootPart.CFrame = Getdownplayerss().HumanoidRootPart.CFrame+Vector3.new(0,-7,0)
                    wait(10)
                end
            end
        end
    ...
    local function revive()
        if Getdownplayers() ~= nil then
            local downplayers = Getdownplayers()
            if downplayers ~= nil and downplayers:FindFirstChild('HumanoidRootPart') then
                task.spawn(function()
                    while task.wait() do
                        if localplayer.Character then
                            pcall(function()
                                workspace.Game.Settings:SetAttribute("ReviveTime", 2.2)
                                localplayer.Character:FindFirstChild('HumanoidRootPart').CFrame = CFrame.new(downplayers:FindFirstChild('HumanoidRootPart').Position.X, downplayers:FindFirstChild('HumanoidRootPart').Position.Y - 8, downplayers:FindFirstChild('HumanoidRootPart').Position.Z)
                                task.wait(1)
                                game:GetService("ReplicatedStorage").Events.Revive.RevivePlayer:FireServer(tostring(downplayers), false)
                                task.wait(2.2)
                                game:GetService("ReplicatedStorage").Events.Revive.RevivePlayer:FireServer(tostring(downplayers), true)
                                game:GetService("ReplicatedStorage").Events.Revive.RevivePlayer:FireServer(tostring(downplayers), true)
                                game:GetService("ReplicatedStorage").Events.Revive.RevivePlayer:FireServer(tostring(downplayers), true)
                                game:GetService("ReplicatedStorage").Events.Revive.RevivePlayer:FireServer(tostring(downplayers), true)
                                game:GetService("ReplicatedStorage").Events.Revive.RevivePlayer:FireServer(tostring(downplayers), true)
    ...
    task.spawn(function()
        while true and task.wait() do
            if (not getgenv().config.AutoObjectives and getgenv().config.AutoRevive and localplayer.Character ~= nil and localplayer.Character:FindFirstChild("HumanoidRootPart")) or 
               (getgenv().config.AutoObjectives and getgenv().config.AutoRevive and localplayer.Character ~= nil and localplayer.Character:FindFirstChild("HumanoidRootPart") and not localplayer.Character:GetAttribute("Downed") and getobj() ~= nil and getobj():WaitForChild("Use") and game:GetService("Workspace").Game.Stats:GetAttribute("RoundStarted") == true and localplayer.PlayerGui:FindFirstChild("HUD") and localplayer.PlayerGui:FindFirstChild("HUD"):FindFirstChild("Objectives") and localplayer.PlayerGui:FindFirstChild("HUD"):FindFirstChild("Objectives"):FindFirstChild("Frame") and localplayer.PlayerGui:FindFirstChild("HUD"):FindFirstChild("Objectives"):FindFirstChild("Frame"):FindFirstChild("Objective") and localplayer.PlayerGui:FindFirstChild("HUD"):FindFirstChild("Objectives"):FindFirstChild("Frame"):FindFirstChild("Objective").TextColor3 == Color3.fromRGB(137, 255, 141)) then
                if not localplayer.Character.HumanoidRootPart:FindFirstChild("BodyGyro") and not localplayer.Character.HumanoidRootPart:FindFirstChild("BodyVelocity") then
                ...
                revive()
            elseif localplayer.Character ~= nil and localplayer.Character:FindFirstChild("HumanoidRootPart") then
                if localplayer.Character.HumanoidRootPart:FindFirstChild("BodyGyro") and localplayer.Character.HumanoidRootPart:FindFirstChild("BodyVelocity") then
                    localplayer.Character.HumanoidRootPart:FindFirstChild("BodyGyro"):Destroy()
    ...
    local monsterESP = {}
    monsterESP.attached = {}
    monsterESP.__index = monsterESP

    local z = game:GetService("Workspace").CurrentCamera
    game:GetService("Workspace"):GetPropertyChangedSignal("CurrentCamera"):Connect(function()
        z = game:GetService("Workspace").CurrentCamera
    end)
    local C = z.WorldToViewportPoint

    function monsterESP.new(D)
        local E = setmetatable({instance = D, destructed = false}, monsterESP)
        E.instance.AncestryChanged:Connect(function(F, G)
            if G == nil then
                E:destroy()
            end
        end)
    ...
    function monsterESP.constructor(D, E)
        D.quad = Drawing.new("Quad")
        D.root = (E.PrimaryPart or E:WaitForChild("HRP", 5))
        task.defer(D.onStart, D)
        return D
    end

    function monsterESP.onStart(D)
        monsterESP.attached[D.instance] = D
        local E = D.quad
        E.Visible = false
        E.Transparency = 0.75
        E.Color = Color3.fromRGB(128,0,128)
        E.Thickness = 2
    end

    function monsterESP.render(D)
        local E = D.quad
        local F = D.root
        if getgenv().config.BoxESPMonster then
            local G = F.CFrame
            local H = F.Size
            local I = C(z, (G * CFrame.new(H.X , H.Y * 0.8, 0)).Position)
            local J = C(z, (G * CFrame.new(-H.X , H.Y * 0.8, 0)).Position)
            local K = C(z, (G * CFrame.new(-H.X , -H.Y * 0.8, 0)).Position)
            local L = C(z, (G * CFrame.new(H.X , -H.Y * 0.8, 0)).Position)
            if math.min(I.Z, J.Z, K.Z, L.Z) > 0 then
                E.PointA = Vector2.new(J.X, J.Y)
                E.PointB = Vector2.new(I.X, I.Y)
                E.PointC = Vector2.new(L.X, L.Y)
                E.PointD = Vector2.new(K.X, K.Y)
                E.Visible = true
            else
                E.Visible = false
            end
    ...
    function monsterESP.destroy(D)
        if D.destructed then
            return
        else
            D.destructed = true
        end
        D.quad:Remove()
        monsterESP.attached[D.instance] = nil
    end

    game:GetService("RunService").RenderStepped:Connect(function()
        for D, E in pairs(monsterESP.attached) do
            E:render()
        end
    end)

    for i,v in pairs(game:GetService("Workspace").Game.Players:GetChildren()) do
        if v:FindFirstChild("HRP") then
            monsterESP.new(v)
        end
    end
    ...
    local wrsp = game.Workspace
    local curcam = wrsp.CurrentCamera
    local cam = wrsp.CurrentCamera
    local wtvp = curcam.worldToViewportPoint
    local rs = game:GetService("RunService")
    
    -- Line ESP
    for i,v in pairs(game.Players:GetChildren()) do
        local trac = Drawing.new("Line")
        trac.Color = Color3.new(1,1,1)
        trac.Thickness = 1
        trac.Visible = false
        trac.Transparency = 1
        trac.Thickness = 1
        ...
        function tracer()
            rs.Stepped:Connect(function()
                if v.Character ~= nil and not v:GetAttribute("Dead") and v.Character:FindFirstChild("Humanoid") ~= nil and v.Character:FindFirstChild("HumanoidRootPart") ~= nil and v ~= plr then
                    local vec, scr = cam:worldToViewportPoint(v.Character.HumanoidRootPart.Position)
                    trac.From = Vector2.new(cam.ViewportSize.X/2, cam.ViewportSize.Y)
                    trac.To = Vector2.new(vec.X, vec.Y)
                    if scr then
                        if v.Character:GetAttribute("Downed") and not v:GetAttribute("HasGun") then
                            trac.Color = Color3.fromRGB(255,0,0)
                        elseif v:GetAttribute("HasGun") then
                            trac.Color = Color3.fromRGB(128,0,128)
                        else
                            trac.Color = Color3.fromRGB(0,255,0)
                        end
                        trac.Visible = getgenv().config.LineESP
                    else
                        trac.Visible = false
                    end
                else
    ...
    -- Box and Name ESP
    local headout = Vector3.new(0,0.5,0)
    local legout = Vector3.new(0,5,0)
    
    for i,v in pairs(game.Players:GetChildren()) do
        local boxline = Drawing.new("Square")
        boxline.Visible = false
        boxline.Color = Color3.fromRGB(255, 255, 255)
        boxline.Thickness = 1
        ...
        function applyesp()
            rs.Stepped:Connect(function()
                if v.Character ~= nil and not v:GetAttribute("Dead") and v.Character:FindFirstChild("Head") and v.Character:FindFirstChild("HumanoidRootPart") ~= nil and v.Character:FindFirstChild("Humanoid") ~= nil and v ~= plr then
                    nametext.Text = v.Name
                    local vec, scr = cam:worldToViewportPoint(v.Character.HumanoidRootPart.Position)
                    local hedpos = cam:worldToViewportPoint(v.Character.Head.Position)
                    local hrp = v.Character.HumanoidRootPart
                    local head = v.Character.Head
                    local hrppos, hrpvis = wtvp(curcam, hrp.Position)
                    local headpos = wtvp(curcam, head.Position + headout)
                    local legpos = wtvp(curcam, hrp.Position - legout)
                    
                    if scr then
                        boxline.Size = Vector2.new(2000/hrppos.Z, headpos.Y - legpos.Y)
                        boxline.Position = Vector2.new(hrppos.X-boxline.Size.X/2, hrppos.Y - boxline.Size.Y/2)
                        nametext.Position = Vector2.new(hedpos.X,hedpos.Y-60)
                        
                        if getgenv().config.BoxESP then
                            if v.Character:GetAttribute("Downed") and not v:GetAttribute("HasGun") then
                                boxline.Color = Color3.fromRGB(255,0,0)
                            elseif v:GetAttribute("HasGun") then
                                boxline.Color = Color3.fromRGB(128,0,128)
                            else
                                boxline.Color = Color3.fromRGB(0,255,0)
                            end
                        ...
                        if getgenv().config.NameESP then
                            ...
                            elseif v:GetAttribute("HasGun") then
                                nametext.Color = Color3.fromRGB(128,0,128)
                            else
    ...
    -- WalkSpeed and JumpPower Hook
    local old
    old = hookmetamethod(game,"__namecall",newcclosure(function(self,...)
        local Args = {...}
        local method = getnamecallmethod()
        if tostring(self) == 'Communicator' and method == "InvokeServer" and Args[1] == "update" then
            return getgenv().config.WalkSpeed, getgenv().config.JumpPower
        end
        return old(self,...)
    end))
